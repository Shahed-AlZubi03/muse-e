...
| `GET` | `/api/sessions` | ✓ | List the user's past conversation sessions |
| `POST` | `/api/quiz/generate` | ✓ | Generate a quiz from a session |
| `POST` | `/api/quiz/:quizId/answer` | ✓ | Submit one answer, receive instant feedback |
| `POST` | `/api/quiz/:quizId/complete` | ✓ | Finalise the quiz and receive the score |
| `GET` | `/api/quiz/session/:sessionId` | ✓ | Fetch quizzes belonging to a session |
...
Accepted image formats: **JPEG, PNG, GIF, WebP** (max 10 MB). The format is detected from the file's magic bytes, so a wrong or generic `Content-Type` from the browser is handled correctly; anything else returns `400`.
...
curl -X POST http://localhost:3000/api/ask \
  ...
  -F "sessionId=6a8555bf3f12854ae8477e71" \
  -F "language=en"
```
...
```json
{
  ...
  "sessionId": "6a8555bf3f12854ae8477e71",
  "artworkId": "6a85556bcdd79a4cef245a4d",
  "artwork": {
...
| File uploads | `multer` size cap (10 MB) + magic-byte format check (JPEG/PNG/GIF/WebP) — the client-supplied MIME type is never trusted |
...
- 🚧 Semantic search over artworks — embeddings are generated by `npm run seed:embeddings`, but vector retrieval is not yet wired into the request path (RAG currently retrieves by title/artist match)
- 🚧 Similar-artwork recommendations
- 🚧 Geolocation-based museum discovery
